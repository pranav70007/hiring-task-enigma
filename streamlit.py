...
@lru_cache(maxsize=1000)  
def _fetch_single_node_context(node_id: str, depth: int = 1) -> tuple:
    facts = []
    with driver.session() as session:
        q = (
            "MATCH path=(n:Entity {id:$nid})-[r*1..$depth]-(m:Entity) "
        ...
        recs = session.run(q, nid=node_id, depth=depth)
        for r in recs:
            facts.append((
                node_id,
                r["rel_types"],
...
def fetch_graph_context(node_ids: List[str], neighborhood_depth):
    facts = []
    for nid in node_ids:
        f = _fetch_single_node_context(nid,depth=3)
        facts.append({
                "source": f[0],
                "rel": f[1],
                "target_id": f[2],
...
def build_prompt(user_query, pinecone_matches, graph_facts):
    ...
    for source, facts in grouped_facts.items():
        ...
        for f in facts:
            ...
            desc = f"which {f['target_desc'][:100]}..." if f['target_desc'] else ""
